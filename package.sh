#!/bin/bash
# package.sh - Create installers for DiceyStarCluster
# Run this from the DiceyStarCluster/ directory

APP_NAME="DiceyStarCluster"
...
dotnet publish DiceyStarCluster.csproj -c Release -r linux-x64 --self-contained true -p:PublishSingleFile=true -p:PublishTrimmed=true
...
for arch in "osx-arm64" "osx-x64"; do
    dotnet publish DiceyStarCluster.csproj -c Release -r $arch --self-contained true -p:PublishSingleFile=true
    if [ -d "$PUBLISH_DIR/$arch/publish" ]; then
        # Create the .app bundle structure
        # We name it simply DiceyStarCluster.app for the zip, but distinguish the zip filename
...
dotnet publish DiceyStarCluster.csproj -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true
